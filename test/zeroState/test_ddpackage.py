...
class TestDDPackage(TestCase):
    def test_create_zero_state(self):
        ...
        #print("------------")
        x = dd.create_single_qubit_gate(4, 0, np.array([[0, 1], [1, 0]], dtype=np.complex128))
        hadamard = dd.create_single_qubit_gate(4, 1, 1/np.sqrt(2)*np.array([[1, 1], [1, -1]], dtype=np.complex128))
        #print(dd.gate_print_two_qubit_debug(x))
        #gate2 = dd.create_single_qubit_gate(2, 1, np.array([[0, 1], [1, 0]], dtype=np.complex128))
        cx = dd.create_controlled_single_qubit_gate(4, [1], 0, np.array([[0, 1], [1, 0]]))
